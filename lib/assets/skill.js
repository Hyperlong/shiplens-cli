...
2. \`List Account Projects\`: Query all analytics projects and associated App IDs registered under your account (\`projects list\`).
3. \`Update Project Information\`: Modify project name, description, and industry taxonomy (\`projects update\`).
4. \`Update Business Context Dictionary\`: Re-scan frontend pages, buttons, and interaction flows (\`context generate\`).
5. \`Link Project to Account & Start Analytics\`: Bind current project to authenticated account (\`projects bind\`).
6. \`Remove SDK & Tracking Code\`: Eject tracking code and uninstall \`@shiplens/sdk\` dependency (\`clean sdk\`).
7. \`Delete Business Context Files\`: Clean up local \`.shiplens/contexts/\` dictionary files (\`clean context\`).
8. \`Delete Cloud Project\`: Permanently destroy cloud project and all historical event data (\`projects delete\`).
