...
**Project Operations**:
1. \`List Products Integrated with Analytics\`: Query all products integrated with data analytics and associated App IDs registered under your account with full details (\`projects list\`).
2. \`Integrate Local Project with Analytics\`: Register product under account, install SDK into local project, inject tracking code, and generate context files (\`init\`).
3. \`Update Product Profile\`: Modify registered product name, description, and industry taxonomy (\`projects update\`).
4. \`Update Local Business Context\`: Re-scan frontend pages & buttons to refresh local business descriptions (\`context generate\`).
5. \`Remove SDK and Local Context Files\`: Eject Shiplens SDK from project and clean up generated context files (\`clean all\`).
6. \`Permanently Delete Product and Data\`: Permanently delete cloud product registration and telemetry statistics, without deleting local code (\`projects delete\`).
