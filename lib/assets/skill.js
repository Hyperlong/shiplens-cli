...
**Project Operations**:
1. \`Integrate Local Project with Analytics\`: Register product under account, install SDK into local project, inject tracking code, and generate UI business context dictionary (\`init\`).
2. \`List Registered Products\`: Query all integrated products and associated App IDs registered under your account (\`projects list\`).
3. \`Update Product Profile\`: Modify registered product name, description, and industry taxonomy (\`projects update\`).
4. \`Update Local Business Context Dictionary\`: Re-scan local frontend pages, buttons, and interaction flows (\`context generate\`).
5. \`Link Local Project to Registered Product\`: Connect local project code to registered cloud product (\`projects bind\`).
...
8. \`Permanently Delete Product and Data\`: Permanently delete cloud product profile and historical event data, without deleting local code (\`projects delete\`).
