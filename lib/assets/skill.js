...
**Project Operations**:
1. \`Integrate Local Project with Analytics\`: Create cloud analytics dashboard, install SDK into local project, inject tracking code, and generate UI business context dictionary (\`init\`).
2. \`List Cloud Analytics Dashboards\`: Query all analytics dashboards and associated App IDs registered under your account (\`projects list\`).
3. \`Update Dashboard Information\`: Modify product name, description, and industry taxonomy (\`projects update\`).
4. \`Update Local Business Context Dictionary\`: Re-scan local frontend pages, buttons, and interaction flows (\`context generate\`).
5. \`Link Local Project to Cloud Dashboard\`: Connect local project code to authenticated cloud dashboard (\`projects bind\`).
6. \`Remove SDK from Local Project\`: Eject tracking code and uninstall \`@shiplens/sdk\` dependency (\`clean sdk\`).
7. \`Delete Local Context Files\`: Clean up local \`.shiplens/contexts/\` dictionary files (\`clean context\`).
8. \`Permanently Destroy Cloud Dashboard\`: Permanently destroy cloud analytics dashboard and historical event data, without deleting local code (\`projects delete\`).
