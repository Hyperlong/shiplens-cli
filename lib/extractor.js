...
/**
 * 结构化判断是否为纯代码/语法片段（严格过滤 AST 中的语法残留）
 */
function isCodeOrSyntaxSnippet(str) {
  if (!str || typeof str !== 'string') return true;
  const t = str.trim();
  if (t.length < 2) return true;
  if (/^(?:case\s+|return\s+|switch\s*\(|const\s+|let\s+|var\s+|import\s+|export\s+|function\s+|class\s+|this\.|try\s*\{|catch\s*\(|if\s*\(|else\s*\{|for\s*\(|while\s*\(|\$\s*`;|\/\*|\*\/)/i.test(t)) {
    return true;
  }
  if (/[{}();=><]{3,}/.test(t) || t.includes('=>') || t.includes('===') || t.includes('!==') || t.includes('::') || t.includes('case BlockType')) {
    return true;
  }
  return false;
}
 ...
 * 自然语言标题归一化与多语言消歧
 */
function normalizeLocalizedTitle(title, isZh = true) {
  if (!title || isCodeOrSyntaxSnippet(title)) return 'Home (首页)';
  let clean = title.trim();
  if (isZh) {
    const DICT = {
      'Dateikonverter': '文件转换器',
      'Base64-Dateikonverter': 'Base64 文件转换器',
      'Konvertiere Strings': '转换字符串',
      'JWT-Parser': 'JWT 令牌解析器',
      'UUID-Generator': 'UUID 唯一标识符生成器',
      'SQL-Prettify': 'SQL 语句美化工具',
      'Regex-Tester': '正则表达式测试器',
      'Color-Converter': '颜色格式转换器',
      'Crontab-Generator': 'Crontab 定时任务生成器',
      'JSON-Formatter': 'JSON 格式化工具',
      'Inter Variable': 'API 调试与请求工作区',
    };
    for (const [k, v] of Object.entries(DICT)) {
      if (clean.toLowerCase().includes(k.toLowerCase())) {
        clean = clean.replace(new RegExp(k, 'i'), v);
      }
    }
  }
  return clean;
...
/**
 * 结构化提取 README 中的核心功能特性、API 方法、鼠标按键与操作指令 (Layer 1 增强)
 * @param {string} rootDir 项目根目录
 * @returns {Array<{ id: string, tag: string, text: string, selector: string, category: string, action_hint: string, source_loc: string, is_readme_feature: boolean, role_type?: string }>}
 */
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (/^#+\s*/.test(line)) {
          ...
          if (/license|contributing|sponsors|credits|donate|setup|install|table of contents/i.test(headingText)) {
            inFeatureSection = false;
            continue;
          }

          if (/feature|method|mouse|key|event|option|command|function|control|component|hotkey|usage|module|gameplay|characteristic|operation|action|button|rule|tree|list|hash|graph|sort|algorithm|功能|特性|工具|按键|操作|方法/i.test(headingText)) {
            inFeatureSection = true;
            currentCategory = headingText;

            // 标题本身也可以作为一个特征/操作分类
            if (headingText.length > 2 && headingText.length < 50 && !isCodeOrSyntaxSnippet(headingText)) {
        ...
        if (inFeatureSection) {
          // 匹配列表条目 (如 - Drag Nodes, - `del key` to remove, - Zoom in / out)
          ...
          if (listMatch) {
            ...
            if (cleanText.length > 2 && cleanText.length < 120 && !cleanText.startsWith('http') && !isCodeOrSyntaxSnippet(cleanText)) {
              ...
              let roleType = 'action';
              let tagType = 'feature-action';
              if (/del|remove|delete|clear|reset/i.test(cleanText)) roleType = 'action';
              if (/key|click|wheel|mouse|press|drag/i.test(cleanText)) tagType = 'gesture-control';
              if (/export|download|save|submit|convert/i.test(cleanText)) roleType = 'action';
          ...
          // 匹配 API 方法调用或声明 (如 editor.export(), editor.import(data), editor.addNode(...))
          const codeCallMatch = line.match(/(?:editor|drawflow|[a-zA-Z0-9_]+)\.([a-zA-Z0-9_]{3,30})\s*\(/) || line.match(/^`?([a-zA-Z0-9_]{3,30})\s*\([^)]*\)`?/);
          if (codeCallMatch) {
            const fnName = codeCallMatch[1];
            if (!/^(log|warn|error|info|assert|push|slice|indexOf|includes|forEach|map|filter|require|import)/i.test(fnName)) {
              ...
              if (!features.some((f) => f.id === `act_fn_${slug}`)) {
                features.push({
                  ...
                  category: currentCategory,
                  action_hint: `Call ${fnName}() method (${currentCategory})`,
                  source_loc: `${cand}:${i + 1}`,
                  is_readme_feature: true,
                  role_type: 'action',
                });
...
function extractPageSkeleton(filePath, routePath, pageName, rootDir = '', componentFiles = []) {
  ...
  for (const targetFile of allFiles) {
    try {
      ...
      for (const h of richContent.headings) if (!isCodeOrSyntaxSnippet(h) && !allHeadings.includes(h)) allHeadings.push(h);
      for (const a of richContent.alerts) if (!isCodeOrSyntaxSnippet(a) && !allAlerts.includes(a)) allAlerts.push(a);
      for (const t of richContent.tooltips) if (!isCodeOrSyntaxSnippet(t) && !allTooltips.includes(t)) allTooltips.push(t);
      for (const l of richContent.form_labels) if (!isCodeOrSyntaxSnippet(l) && !allFormLabels.includes(l)) allFormLabels.push(l);
      for (const p of richContent.placeholders) if (!isCodeOrSyntaxSnippet(p) && !allPlaceholders.includes(p)) allPlaceholders.push(p);
      for (const c of richContent.card_titles) if (!isCodeOrSyntaxSnippet(c) && !allCardTitles.includes(c)) allCardTitles.push(c);
      for (const f of richContent.faqs) if (!allFaqs.some((existing) => existing.q === f.q)) allFaqs.push(f);
      for (const b of richContent.feature_bullets) if (!isCodeOrSyntaxSnippet(b) && !allFeatureBullets.includes(b)) allFeatureBullets.push(b);
      for (const d of richContent.descriptions) if (!isCodeOrSyntaxSnippet(d) && !allDescriptions.includes(d)) allDescriptions.push(d);
  ...
  // 双向混合补偿机制（面向类库、画布状态机及按钮较少的场景）
  if (allButtons.length < 15 && rootDir) {
    ...
    for (const rf of readmeFeatures) {
      ...
      if (!seenButtonKeys.has(uniqueKey)) {
        seenButtonKeys.add(uniqueKey);
        allButtons.push(rf);
        if (!allFeatureBullets.includes(rf.text)) {
          allFeatureBullets.push(rf.text);
        }
      }
      if (allButtons.length >= 60) break;
    }
  }

  if (toolMeta.title) {
    toolMeta.title = normalizeLocalizedTitle(toolMeta.title, true);
  }

  return {
    path: routePath,
    name: normalizeLocalizedTitle(pageName, true),
    tool_meta: Object.keys(toolMeta).length > 0 ? toolMeta : undefined,
    headings: allHeadings.filter(h => !isCodeOrSyntaxSnippet(h)).slice(0, 12),
    alerts: allAlerts.filter(a => !isCodeOrSyntaxSnippet(a)).slice(0, 8),
    tooltips: allTooltips.filter(t => !isCodeOrSyntaxSnippet(t)).slice(0, 8),
    form_labels: allFormLabels.filter(l => !isCodeOrSyntaxSnippet(l)).slice(0, 15),
    placeholders: allPlaceholders.filter(p => !isCodeOrSyntaxSnippet(p)).slice(0, 10),
    card_titles: allCardTitles.filter(c => !isCodeOrSyntaxSnippet(c)).slice(0, 8),
    faqs: allFaqs.slice(0, 10),
    feature_bullets: allFeatureBullets.filter(b => !isCodeOrSyntaxSnippet(b)).slice(0, 20),
    descriptions: allDescriptions.filter(d => !isCodeOrSyntaxSnippet(d)).slice(0, 8),
