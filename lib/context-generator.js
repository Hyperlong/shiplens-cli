...
function normalizeControlId(btn, routePrefix, seenIds) {
  ...
  const section = (btn.section || btn.card_title || '').trim();
  ...
  // 父级容器语义辅助消歧（如 Card[title="Input"] vs "Output"）
  let sectionPrefix = '';
  if (section && !label && /input|output|source|target|decode|encode|preview|result/i.test(section)) {
    const sSlug = section.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 8);
    if (sSlug && !actionSlug.startsWith(sSlug)) {
      sectionPrefix = `${sSlug}_`;
    }
  }

  // 统一前缀：按钮、输入框、上传统一用 btn_ 作为 Shiplens 点击/操作埋点前缀
  let candidateId = `btn_${routePrefix}_${sectionPrefix}${actionSlug}`;
...
/**
 * 页面功能全景总结 (Page Functional Summary) 高阶业务语义合成引擎 (JTBD & Domain Driven)
 * 结构：
 * 1. 功能全景与业务定义 (包含 1~3 个核心工作流拆解)
 * 2. 业务用途与满足的用户需求 (包含具象的用户需求痛点与开发者设计意图)
 * 3. 输入与输出流 (I/O)
...
function generatePageSummary(page, skeleton, factsheet = {}, isZh = true) {
  ...
  let toolTitle = meta.title || skeleton.headings[0] || page.name || '工作区';
  
  // 标题清洗与消歧
  if (/^(?:case|return|const|import|function|\$)/i.test(toolTitle) || toolTitle.length < 2) {
    toolTitle = factsheet.name || '核心交互工作区';
  }
  if (toolTitle === 'Inter Variable') {
    toolTitle = 'API 调试与请求工作区 (API Workspace)';
  }

  const rawDesc = meta.description || skeleton.descriptions[0] || factsheet.description || '';
  const toolDesc = rawDesc && !/^(?:case|return|const|import)/i.test(rawDesc) ? rawDesc : '';
  const cardTitles = (skeleton.card_titles || []).filter(c => !/^(?:case|return|const)/i.test(c));
  const formLabels = (skeleton.form_labels || []).filter(l => !/^(?:case|return|const)/i.test(l));
  const placeholders = (skeleton.placeholders || []).filter(p => !/^(?:case|return|const)/i.test(p));
  const buttons = skeleton.raw_buttons || [];

  // 提取核心输入项与转化按钮
  const inputLabels = Array.from(new Set(formLabels.concat(placeholders))).slice(0, 5);
  ...
  // 识别页面业务领域特征
  const pLower = (page.path + ' ' + toolTitle + ' ' + toolDesc).toLowerCase();
  const isBase64 = pLower.includes('base64');
  const isJwt = pLower.includes('jwt') || pLower.includes('token');
  const isUuid = pLower.includes('uuid') || pLower.includes('random') || pLower.includes('guid');
  const isFlowCanvas = pLower.includes('drawflow') || pLower.includes('flow') || pLower.includes('canvas') || pLower.includes('excalidraw');
  const isApiDebugger = pLower.includes('hoppscotch') || pLower.includes('request') || pLower.includes('api') || pLower.includes('rest');
  const isChat = pLower.includes('cinny') || pLower.includes('matrix') || pLower.includes('chat') || pLower.includes('message');
  const isSocial = pLower.includes('social') || pLower.includes('post') || pLower.includes('feed') || pLower.includes('bluesky');
  const isFormatter = pLower.includes('format') || pLower.includes('prettify') || pLower.includes('beautify') || pLower.includes('json') || pLower.includes('sql') || pLower.includes('xml') || pLower.includes('yaml');

  if (isZh) {
    let workflows = [];
    if (cardTitles.length >= 2) {
      workflows = cardTitles.slice(0, 3).map((ct, idx) => `${idx + 1}. **${ct}**：支持在当前面板中执行 ${ct} 相关的配置、数据录入与即时转换；`);
    } else if (isBase64) {
      workflows.push('1. **文件转 Base64 (Encode to Base64)**：用户拖拽或选择本地任意文件，即时在客户端读取并编码为 Base64 Data URL 字符串；');
      workflows.push('2. **Base64 转文件 (Decode to File)**：将输入的 Base64 文本根据配置的文件名与扩展名还原解码并直接下载到本地。');
    } else if (isFlowCanvas) {
      workflows.push('1. **节点拓扑编排与连线**：在可视化画布上动态拖拽创建多输入/输出节点并绘制曲线连线；');
      workflows.push('2. **数据流序列化与导出**：一键将整张画布的节点拓扑与连接关系导出为标准 JSON 格式或从数据还原。');
    } else if (isApiDebugger) {
      workflows.push('1. **多协议 API 请求编排**：配置 HTTP Method、URL、Headers、Query 参数与 Body 负载；');
      workflows.push('2. **实时响应核验与集合持久化**：发送请求并查看实时返回的状态码、响应体与耗时，支持保存至团队请求集合。');
    } else if (isChat) {
      workflows.push('1. **房间与 Space 导航**：在侧边栏切换不同频道、私聊会话与群组空间；');
      workflows.push('2. **端到端加密消息收发**：在对话主区域输入富文本/Markdown 消息，通过 Matrix 协议安全发送与接收媒体文件。');
    } else if (isFormatter) {
      workflows.push('1. **代码与数据格式化**：接收用户粘贴的压缩/杂乱文本，按照标准语法缩进即时美化；');
      workflows.push('2. **结果复制与语法校验**：高亮语法错误，并支持一键将美化后的文本完整复制到剪贴板。');
    } else if (cardTitles.length === 1) {
      workflows.push(`1. **${cardTitles[0]}**：用户输入或配置相关参数，即时触发处理并获取结果；`);
    } else {
      workflows.push(`1. **参数配置与业务执行**：用户在当前面板录入输入配置，通过核心操作控件即时完成业务流转与产出；`);
    }

    const workflowStr = workflows.join('\n> ');

    // 提炼具体的用户需求与设计意图
    let userNeed = `解决用户在${toolTitle}场景下的数据处理、格式转换或系统配置需求，提供免安装、纯前端运行的极速工具；`;
    let devIntent = `开发者通过模块化卡片与清晰的表单布局，引导用户以“输入 $\\rightarrow$ 配置 $\\rightarrow$ 导出”的标准闭环高效完成任务。`;

    if (isBase64) {
      userNeed = '解决开发者在嵌入小图片至 CSS/HTML、解析 API 传输的 Base64 附件时，无需依赖外部接口、数据不离本地的安全转换诉求；';
      devIntent = '采用纯客户端内存编解码设计，保障用户隐私与敏感文件安全，实现即放即转、一键复制/下载的极致效率闭环。';
    } else if (isFlowCanvas) {
      userNeed = '满足自动化流程编排、低代码逻辑流、规则引擎配置等场景下对“轻量、零依赖、易嵌入”的可视化连线画布需求；';
      devIntent = '通过直观的鼠标按键手势（Del 键删除、滚轮缩放、右键菜单）与简洁的 API，降低二次开发与流程拓扑建模门槛。';
    } else if (isApiDebugger) {
      userNeed = '解决前后端研发与测试人员快速调试接口、验证接口返回、组织 API 集合与环境隔离的高频协作诉求；';
      devIntent = '采用极简轻量 UI 与响应式双栏布局，提供无干扰的 Zen 模式与毫秒级实时请求收发体验。';
    } else if (isChat) {
      userNeed = '满足个人与团队在去中心化开源协议下进行高安全性、端到端加密即时通讯与文件传输的需求；';
      devIntent = '打造现代优雅、低学习成本的通讯界面，兼顾 Space 多维空间组织与强加密安全保障。';
    } else if (isUuid) {
      userNeed = '解决开发测试过程中需要批量生成高碰撞阻断能力的 UUID v4/v5 唯一标识符的诉求；';
      devIntent = '提供数量调节与命名空间参数，一键复制生成清单，提升数据构造效率。';
    }
    ...
    let outputSummary = '计算生成的结果数据、复制到剪贴板的令牌/字符串、或即时渲染的视图。';
    if (isBase64) {
      outputSummary = '解码还原的本地二进制文件（带 MIME 类型校验）、格式化 Base64 字符串（一键复制至剪贴板）、以及图片原地实时渲染预览。';
    } else if (isFlowCanvas) {
      outputSummary = '结构化的 JSON 数据流定义、画布内即时渲染的 SVG 动态曲线与节点拓扑状态。';
    } else if (isApiDebugger) {
      outputSummary = '服务器返回的实时 HTTP 状态码、响应 Header 与 JSON/XML 响应体，或持久化保存的请求集合。';
    } else if (isChat) {
      outputSummary = '加密发送至 Matrix 房间的消息事件、实时接收的聊天流动态与多媒体预览视图。';
    } else if (coreOECNames.some(n => /download/i.test(n))) {
      outputSummary = '下载还原后的本地文件、生成文本复制到剪贴板、或页面内即时预览渲染。';
    }
  ...
  return [
    ...
    `> - **User Need**: Enables rapid in-browser processing and conversion without external software dependencies.  `,
    ...
    `> - **Input**: Parameters, text payloads, or uploaded local binary files.  `,
...
function generateProjectContext(dir = process.cwd(), appId, projectName = 'My App', framework = 'custom', lang = 'zh') {
  ...
  // Standard storage paths: .shiplens/contexts/<app_id>.md and .shiplens/contexts/<app_id>.json
  const contextDir = path.join(dir, '.shiplens', 'contexts');
  fs.mkdirSync(contextDir, { recursive: true });
  const targetFilePath = path.join(contextDir, `${appId}.md`);
  const targetJsonPath = path.join(contextDir, `${appId}.json`);

  fs.writeFileSync(targetFilePath, markdownContent, 'utf8');

  // 构建 <app_id>.json 结构化 UI 字典
  const routesObj = {};
  for (const p of pageDetails) {
    const routePrefix = generateRoutePrefix(p.path);
    const oecActions = p.controls.filter((c) => c.conversion_role === '核心转化').map((c) => c.id);
    const controlsByText = {};
    const controlsById = {};

    for (const c of p.controls) {
      const textKey = (c.text || c.label || c.id || '').replace(/\[State:[^\]]+\]/g, '').trim();
      const controlData = {
        id: c.id,
        role: c.conversion_role,
        tag: c.tag || 'button',
        source_loc: c.source_loc || undefined,
        description: c.description || '',
      };
      if (textKey && !controlsByText[textKey]) {
        controlsByText[textKey] = controlData;
      }
      controlsById[c.id] = {
        text: textKey || c.id,
        role: c.conversion_role,
    ...
    routesObj[p.path] = {
      prefix: routePrefix,
      title: p.meta.title || p.headings[0] || p.name || p.path,
      category: p.meta.category || undefined,
      source_file: p.relFilePath,
      oec_actions: oecActions,
      controls_by_text: controlsByText,
      controls_by_id: controlsById,
    };
  }

  const jsonObject = {
    $schema: 'https://shiplens.dev/schema/ui-dictionary-v1.json',
    app_id: appId,
    project_name: projectName,
    framework: activeFramework,
    generated_at: now,
    total_pages: pageDetails.length,
    total_controls: totalButtonsCount,
    total_oec: totalOECCount,
    routes: routesObj,
  };

  fs.writeFileSync(targetJsonPath, JSON.stringify(jsonObject, null, 2), 'utf8');

  return {
    filePath: targetFilePath,
    relPath: path.relative(dir, targetFilePath).replace(/\\/g, '/'),
    jsonPath: targetJsonPath,
    relJsonPath: path.relative(dir, targetJsonPath).replace(/\\/g, '/'),
...
/**
 * 获取业务上下文 Markdown 文件的标准存储路径 (.shiplens/contexts/<app_id>.md)
 */
...
/**
 * 获取机器结构化 UI 字典 JSON 文件的标准存储路径 (.shiplens/contexts/<app_id>.json)
 */
function getContextJsonPath(appId, customFile = null, dir = process.cwd()) {
  if (customFile) return path.resolve(dir, customFile);
  return path.join(dir, '.shiplens', 'contexts', `${appId}.json`);
}
