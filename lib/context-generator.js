...
function inferButtonIntent(btn, page) {
  ...
  if (t.includes('clear') || t.includes('清除') || t.includes('reset') || t.includes('重置')) {
    return 'Clear all recorded historical data and price inputs';
  }
  if (t.includes('copy') || t.includes('复制') || t.includes('share') || t.includes('分享')) {
    return 'Copy prediction link or share chart image';
  }
  if (t.includes('pattern') || t.includes('图案') || t.includes('模式') || t.includes('chart') || t.includes('走势')) {
    return 'Toggle price prediction pattern or chart visualization mode';
  }
