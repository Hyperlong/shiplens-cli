...
const { handleSkill } = require('./commands/skill');
...
Core Commands:
  ...
  auth          Authentication & credentials (status, login, set, secret, whoami, logout, bind, mcp-config, configure)
  projects      Project management (list, bind, delete)
  clean         Clean up SDK tracking code or local context files (sdk, context, all)
  guide         Display operations guide, capability catalog, and prompts URL
  ...
  skill         Install and manage Shiplens Agent Skills (install [--global|--local], status)
...
skill Subcommands:
  skill install [--global]             Install standard Skill into detected AI IDEs (Antigravity, Cursor, etc.)
  skill install --local                Install Skill into current project workspace (.agents/skills)
  skill install --target <ide>         Install into specific IDE (antigravity/cursor/windsurf/claude/all)
  skill status                         Check Skill installation status across supported IDEs

auth Subcommands:
  auth status                          Check credential validity
  auth login [--browser]               Authenticate CLI via web browser login
...
projects Subcommands:
  projects list                        List all analytics projects under authenticated account
  projects update [--name/--desc/...]  Update project name, description, and industry taxonomy
  projects bind [--name <name>]        Link current local project to authenticated account
  projects delete [--force]            Delete cloud project and all historical event statistics

clean Subcommands:
  clean sdk [--force]                  Eject tracking code and uninstall @shiplens/sdk
  clean context [--force]              Delete local .shiplens/contexts/ dictionary files
  clean all [--force]                  Clean up all local tracking code, configs, and contexts
  ...
  try {
    switch (command) {
      ...
      case 'projects':
        ...
        break;
      case 'clean':
        await handleClean(subcommand || 'all', args.slice(2), flags, ctx);
        break;
      case 'guide':
        await handleGuide(cmdArgs, flags, ctx);
        break;
      case 'query':
      ...
      case 'doctor':
        ...
        break;
      case 'skill':
        await handleSkill(subcommand || 'install', args.slice(2), flags, ctx);
        break;
      case 'context':
