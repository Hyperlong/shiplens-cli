  ...
  if (pages.length === 0) {
    ...
    for (const cDir of candidateDirs) {
      if (fs.existsSync(cDir)) {
        const files = scanFiles(cDir, ['.tsx', '.jsx', '.vue', '.html', '.svelte', '.js', '.ts']);
        for (const file of files) {
          const base = path.basename(file, path.extname(file));
          // 识别页面或核心应用组件 (如 App, Main, Index, Home, Pricing 等)
          if (/^(App|Main|Index|Page|View|Screen|Home|Pricing|Login|Dashboard|Checkout|Register|Setting|About)/i.test(base)) {
            const cleanName = base.toLowerCase().replace(/(page|view|screen)$/i, '');
            const routePath = cleanName === 'home' || cleanName === 'index' || cleanName === 'app' || cleanName === 'main' ? '/' : `/${cleanName}`;
            pages.push({
              path: routePath,
              filePath: file,
              name: base === 'App' || base === 'index' || base === 'main' ? 'Home / App' : base
            });
          }
        }
        if (pages.length > 0) {
