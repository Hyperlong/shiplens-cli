...
function scanSinglePackagePages(rootDir) {
  ...
  if (pages.length === 0) {
    ...
    if (htmlFiles.length > 0) {
      ...
      // 纯 JS/TS 库或单画布项目（如 Drawflow, pokersolver, dwv, chess.js 等）
      let jsFiles = scanFiles(rootDir, ['.js', '.ts'], IGNORE_DIRS).filter((f) => !f.includes('.min.js') && !f.includes('.test.'));
      // 检查 examples/, docs/, demo/ 是否包含 demo HTML 或脚本
      const demoDirs = ['docs', 'examples', 'demo', 'sample'].map((d) => path.join(rootDir, d)).filter((d) => fs.existsSync(d));
      const demoFiles = demoDirs.flatMap((d) => scanFiles(d, ['.html', '.js', '.ts']));
      ...
      try {
        ...
        if (fs.existsSync(pkgJsonPath)) {
          pkgInfo = JSON.parse(fs.readFileSync(pkgJsonPath, 'utf8'));
        }
      } catch (e) {}

      const customViewTitle = pkgInfo.description
        ? `${formatViewName(pkgInfo.name || path.basename(rootDir))} — ${pkgInfo.description.slice(0, 35)}`
        : `${formatViewName(pkgInfo.name || path.basename(rootDir))} Interactive Workspace`;

      if (jsFiles.length > 0 || demoFiles.length > 0) {
        framework = 'js-library';
        const allAssociatedFiles = [...jsFiles, ...demoFiles];
        const mainJs = allAssociatedFiles.find((f) => /^(index|main|app|drawflow|dwv|chess|pokersolver)\.(html|js|ts)/i.test(path.basename(f))) || allAssociatedFiles[0];
        pages.push({
          path: '/',
          filePath: mainJs,
          name: customViewTitle,
          view_type: 'library_entry',
          component_files: allAssociatedFiles.filter((f) => f !== mainJs),
        });
      } else {
