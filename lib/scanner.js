...
function scanSinglePackagePages(rootDir) {
  ...
  const candidateViewDirs = ['views', 'screens', 'scenes', 'features', 'controllers', 'apps', 'components/views']
...
/**
 * 自动探测项目类型并提取页面及核心业务组件列表（支持 Monorepo 穿透）
...
function detectProjectPages(rootDir) {
  ...
  // 2. 探测 Monorepo 子包 (packages/*, apps/*, projects/*)
  const monorepoContainerDirs = ['packages', 'apps', 'projects'].map((d) => path.join(rootDir, d));
