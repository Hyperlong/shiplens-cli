...
function formatTaxonomySummary(taxonomy) {
  ...
  const genreStr = `${taxonomy.genre.name} / ${taxonomy.genre.name_zh || ''} (${taxonomy.genre.type})`;
  const subgenreStr = taxonomy.subgenre ? `${taxonomy.subgenre.name} / ${taxonomy.subgenre.name_zh || ''}` : '';
...
function resolveTaxonomyFromIDs(genreId, subgenreId, tagIdList = []) {
  ...
  if (!genre) {
    genre = genresMap.get('utilities') || { id: 'utilities', name: 'Utilities', name_zh: '实用工具', type: 'App' };
  }
  if (!subgenre) {
    subgenre = subgenresMap.get('tool_calculators') || { id: 'tool_calculators', name: 'Calculators & Currency Converters', name_zh: '科学计算与汇率换算' };
  }
  ...
  return {
    genre: {
      id: genre.id,
      name: genre.name,
      name_zh: genre.name_zh || genre.name,
      type: genre.type || 'App',
    },
    subgenre: {
      id: subgenre.id,
      name: subgenre.name,
      name_zh: subgenre.name_zh || subgenre.name,
    },
    feature_tags,
